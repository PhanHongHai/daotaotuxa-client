...
function FormTeacher(props) {
	...
	return (
		<Form className="edit-account form-custom" {...formItemLayout} onSubmit={handleSubmit}>
			<Row gutter={16}>
				...
				<Col xs={24} sm={12} md={12}>
					<Form.Item label="Facebook" labelAlign="left">
						...
						})(<Input onChange={onChangeInput} name="introduce" placeholder="facebook/..." />)}
					</Form.Item>
				</Col>
				<Col xs={24} sm={12} md={12}>
				...
				<Col xs={24} sm={12} md={12}>
					<Form.Item label="Facebook" labelAlign="left">
						{form.getFieldDecorator('email', {
							initialValue: info && info.introduce,
						})(<Input onChange={onChangeInput} name="introduce" placeholder="Giới thiệu về bản thân" />)}
					</Form.Item>
				</Col>
				<Col xs={24} sm={12} md={12}>
...
export default Form.create({ name: 'edit-teacher' })(FormTeacher);
